...
namespace QuodAstrum.NPCs
{
    public class NpcDrops : GlobalNPC
    {
        public override void NPCLoot(NPC npc)
        {
            ...
            {
                if (!AstrumWorld.spawnOre)
                {                                                          //Red  Green Blue
                    Main.NewText("Lyrium has developed in the world caverns", 200, 200, 55);  //this is the message that will appear when the npc is killed  , 200, 200, 55 is the text color
                ...
                AstrumWorld.spawnOre = true;   //so the message and the ore spawn does not proc(show) when you kill EoC/npc again
            }
